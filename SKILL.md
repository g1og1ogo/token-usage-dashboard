...
| `--compare-json` | 传入上期 `--json-out` 快照，追加「环比对比」面板（业务线增减、新增/已结束主线、整体口径变化）。快照按期归档成不同文件名，**勿与 `--json-out` 同路径**（脚本会直接报错拦下）；上期快照无归因记录时只给提示、不逐条标「新增」 |
...
**第 5 类的三个边界（改脚本前先读，否则会误判）：**

- 目标**不是 git 仓库**时，「会不会被入库」没有判定语义 → 该类只列出候选、不置退出码。
  （早期版本在非仓库下把 `git check-ignore` 的「不在仓库内」当成「会入库」，
  于是任何合法 `*.html` 都报红。假红会让整道闸门被忽略，比不检查更糟。）
- 目标**是 git 仓库**、但有文件是「随技能发布的模板/示例」而非运行产物时，
  在目标根目录放一份 `.privacy-allow`，一行一个相对路径或 glob，显式豁免：

```bash
echo 'assets/*.html' > .privacy-allow     # 只写中性文件名，该文件会公开
```

- **机器上找不到 `git` 命令**时（未安装 / 不在 PATH），「会不会入库」同样无法判定——
  真实仓库会被 `git rev-parse` 的失败误判成「非仓库」，整类静默跳过、退出码 0，属于假绿。
  脚本此时按**未通过**处理并给出告警；可用环境变量 `GIT=/path/to/git` 指定位置后重试。
  判定原则：**无法判定 ≠ 通过**——假红让人无视闸门，假绿直接把产物发出去，两者都必须消灭。

判据顺序：git 可用性（不可用即未通过）→ `.privacy-allow` 豁免 → `git check-ignore` 已忽略 → 否则「会入库」。
...
- [ ] 模型分布表里同一模型只占一行（`Hy4 preview` / `hy4-preview` 这类写法差异已归一，见 pitfalls 第 11 条）
